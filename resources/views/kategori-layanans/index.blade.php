...
<div class="max-w-3xl space-y-5">
    ...
    <div class="bg-white border border-gray-100 rounded-xl p-6">
        ...
        <form method="POST" action="{{ route('kategori-layanans.store') }}" class="flex gap-3 items-end">
            @csrf
            <div class="flex-1">
                <label class="block text-xs text-gray-500 mb-1.5">Nama kategori</label>
    ...
    <div class="bg-white border border-gray-100 rounded-xl overflow-hidden">
        <div class="px-5 py-3.5 border-b border-gray-100">
            <span class="text-sm font-semibold text-gray-900">Daftar kategori</span>
        </div>
        <table class="w-full">
            <thead>
            ...
            @foreach($categories as $cat)
                <tr class="border-b border-gray-50 hover:bg-gray-50" x-data="{ edit: false }">
                    <td class="px-5 py-3">
                        <span x-show="!edit" class="text-sm font-medium text-gray-900 {{ !$cat->aktif ? 'line-through text-gray-400' : '' }}">
                            {{ $cat->nama }}
                        </span>
                        <form x-show="edit" method="POST" action="{{ route('kategori-layanans.update', $cat) }}"
                              class="flex gap-2 items-center">
                            @csrf @method('PUT')
                            <input type="text" name="nama" value="{{ $cat->nama }}"
                    ...
                    <td class="px-5 py-3 text-center">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                     {{ $cat->aktif ? 'bg-green-50 text-green-700 ring-1 ring-green-200' : 'bg-gray-100 text-gray-500 ring-1 ring-gray-200' }}">
                    ...
                    <td class="px-5 py-3">
                        <div class="flex items-center gap-3" x-show="!edit">
                            ...
                            <form method="POST" action="{{ route('kategori-layanans.toggle', $cat) }}">
                                @csrf @method('PATCH')
                                <button type="submit"
                                        class="text-xs {{ $cat->aktif ? 'text-amber-600 hover:text-amber-900' : 'text-green-600 hover:text-green-900' }} transition-colors">
                                    {{ $cat->aktif ? 'Nonaktifkan' : 'Aktifkan' }}
                                </button>
                            </form>
                        </div>
                    </td>
